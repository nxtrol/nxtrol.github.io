...
variables exceeds the space available on the target 
...
(boolean) variables exceeds the space available on the 
...
and function blocks) exceed the space available on the 
...
Function block shares global data 

and is used by more tasks

The indicated function block is called by more than one 

task but uses global variables with process image. For 

this reason the compiler isn't able to refer to the proper 
...
Invalid datablock type for bit 
...
variables with process image in 
...
allocation error. Instruction shall 

be split.

224 

LogicLab user manual ERROR  
...
Bit access allowed only on bit string data types (BYTE, 
...
Variable name already used into a 
...
Override of a parent method belonging to a locally 
...
Interfaces can not be dereferenced from references/
...
Invalid location index specification The address string of the indicated variable isn't correct, 
...
way (e.g.: contains invalid characters, negative numbers 
...
hexadecimal digits (between 0 and 9 and between A and 
...
and 9, a leading sign + or -, a decimal separator '.' Or a 
...
indication in decimal notation and a time unit between 
...
Time constant exceedes 

maximum value

C4620 

LTime constant exceedes 
...
A non-most significant time unit

exceede its range

C4622 

A non-least significant time unit
...
Initial value must begin on the same line as the variable 
...
error (the errors are normally indicated by red 
...
operator does not allow negated 
...
(SIZEOF operator does not allow 
...
An attempt to write a read-only variable or to read a 
...
Program already assigned to a 

task

The indicated program has been assigned to more than 

one task of the target system.
...
Cyclic declaration of function 

blocks

The indicated function block call itself directly or by 
...
variables of the project (e.g.: there isn't a global variable 
...
target definition file (*.TAR) is missing or incomplete.
...
only for programs (error code valid only for some target 
...
variables can't be used in event 
...
Symbolic initialization of constants 
...
Copy of array or structures not 
...
Copy of data having a large size 

(see threshold in project options)

G0106 

Object oriented features not 
...
supported by target system

G0113 

Undefined interface (with
...
Method redefinition in interface

hierarchy (EXTENDS)

G0122 

Invalid operands for query 
...
Can not load reference/address of 
...
Improper assignment to a 

reference, different type

G0127 

Usage of deprecated pointer 

initialization, use NULL instead
...
Operation between ENUM and 

generic constant

G0134 

Operation requires explicit type 

cast

G0135 

Operation required an implicit 
...
Initialization of constants with 
...
VAR_IN_OUT can be assigned to 

other VAR_IN_OUT only
...
Found invalid instruction in patch 

code, could not set breakpoint/

trigger

G0147 

Variable bit access with variable 
...
Argument types mismatch on 

function call

G0151 

Operand types mismatch on 
...
Implicit conversion of Time to 

LTime

G0159 

Cannot convert an LTime into a 

Time implicitly

G0160 

Invalid operation with a time 

typed operand

G0161 

Operation not allowed on Time 

operand

G0162 

Destination type not supported for 
...
Implicit conversion of DATE to 
...
Cannot convert an LDATE into a 
...
Converting a number into a date 

type

G0172 

Operation not allowed on date 

operand

G0173

Operation between a date type 

operand and a non date type 

operand is not allowed

G0174

Operation between different date

type operands (Date and LDate) is 

not allowed

G0175 

Operation not allowed on TIME or 

LTIME

G0176 

Operation not allowed on DATE or 

LDATE

G0177 

Cannot convert a DATE_AND_

TIME into a DATE implicitly

G0178 

Cannot convert a DATE_AND_

TIME into an LDATE implicitly

G0179 

Cannot convert an LDATE_AND_
...
Implicit conversion of DATE_AND_

TIME to LDATE_AND_TIME

G0182

Cannot convert an LDATE_AND_

TIME into a DATE_AND_TIME 

implicitly

G0183

Operation between a date and 

time type operand and a non date 

and time type is not allowed

G0184

Operation between different date

and time type operands (DT and 

LDT) is not allowed

G0185 

Operation not allowed on date and 

time type operand

G0186 

Operation not allowed on DATE_

AND_TIME or LDATE_AND_TIME

G0187 

Converting a date and time type 

into a string

G0188 

Converting a DATE into a DATE_

AND_TIME

G0189 

Converting a LDATE into a DATE_

AND_TIME

G0190 

Converting a DATE into a LDATE_

AND_TIME

G0191 

Converting a LDATE into a LDATE_
...
Date and time typed parameter 

not allowed

G0196 

Converting a floating point into a
...
Converting a bool into a date and 
...
Converting a date type into a time 
...
Converting a time type into a date 
...
Global variable and ENUM field
...
Assignment not admitted in 

expressions

L1163 

Comments not admitted in 
...
VAR_IN_OUT must be assigned in 
...
It will be saved in the actual PPJX 
...
missing and the original path to 
...
following download to the target 
...
Improper usage of query interface 
...
Missing IL block end marker 

({IL})

S1322 

Function in/out variable doesn’t 
...
must be connected to a step/jump 
...
The communication with the target system failed 

because there is no answer from the system itself. 

More common causes of this problem are wrong cable 

connection, invalid target address in communication 

settings, invalid settings of communication parameters 

(such as baud rate), target system failure.
...
block that has an index greater than the largest index 

supported by the target system. Try to upload and create 

new version of the image file using the "Communication

Upload image file" menu option. If the problem persist,

contact the target system vendor.

T8197 

Invalid target information address Internal compiler error.
...
erasure procedure. Contact the target system vendor for 
...
programming procedure. Contact the target system 
...
If the problem persist and there are other applications 
...
of the target system isn't correct or valid. Try to upload 
